...
interface TenantReportsListProps {
...
export function TenantReportsList({ reports }: TenantReportsListProps) {
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(selectedData, "tenant-reports")
  }
        ...
        <TableBody>
          {reports.map((report) => (
            <TableRow key={report.id}>
              ...
              <TableCell className="text-right">{report.activeLeases}</TableCell>
              <TableCell className="text-right">${report.totalRent.toFixed(2)}</TableCell>
