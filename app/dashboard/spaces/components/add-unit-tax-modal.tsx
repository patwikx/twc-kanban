...
import { createUnitTax } from "@/actions/unit-tax";

const unitTaxFormSchema = z.object({
...
type UnitTaxFormValues = z.infer<typeof unitTaxFormSchema>;

interface AddUnitTaxDialogProps {
  unitId: string;
  users: User[];
  currentUserId: string;
}

export function AddUnitTaxDialog({ unitId, users, currentUserId }: AddUnitTaxDialogProps) {
  const [open, setOpen] = useState(false);
  const form = useForm<UnitTaxFormValues>({
    resolver: zodResolver(unitTaxFormSchema),
    defaultValues: {
  ...
  const { execute: submitForm, loading: isSubmitting } = useAsync(
    async (data: UnitTaxFormValues) => {
      try {
        ...
        await createUnitTax(formData);
        toast.success("Space tax record has been added successfully.");
        setOpen(false);
        form.reset();
      } catch (error) {
        toast.error("Failed to add space tax record.");
      }
    }
  );
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Add Space Tax Record</DialogTitle>
        </DialogHeader>
        <Form {...form}>
